<script lang="ts">
  import { Pane, ThemeUtils, Slider } from "svelte-tweakpane-ui";
  ...
  let {
    ...
    longitudeOfAscendingNode = $bindable(),
    argumentOfPeriapsis = $bindable(),
...
>
  ...
  <Slider
    ...
    bind:value={longitudeOfAscendingNode}
    min={0}
    max={360}
    step={1}
  />
  <Slider
    ...
    bind:value={argumentOfPeriapsis}
    min={0}
    max={360}
    step={1}
  />
